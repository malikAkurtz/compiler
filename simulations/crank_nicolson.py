...
def main():
    ...
    CJ    = 0.05 * C_sum
    CS    = 0.95 * C_sum
    EJ    = EJ_EC * EC          # includes Josephson Capacitance CJ
    ...
    alpha = transmon.anharmonicity / hbar
    
    # ---- Qubit Frequency ----
    f_q     = transmon.frequency
    ...
    # ---- Pulse Amplitude ----
    A_0 = OPTIMAL_AMPLITUDE_SCALE * (transmon.energies[1] - transmon.energies[0])
    
    # ---- Pulse Centers ----
    pulse_centers = np.arange(NUM_PULSES) * T_drive
    
    # DRAG correction operator (σ_y-like in 1-2 subspace)
    n_12_y = np.zeros((n, n), dtype=complex)
    n_12_y[1, 2] = -1j * transmon.n["energy"][1, 2]
    n_12_y[2, 1] =  1j * transmon.n["energy"][2, 1]
    ...
    for i, t in enumerate(t_vec):
        ...
        At_dot   = A_0 * np.sum(-dt_to_pulses[mask] / OPTIMAL_SIGMA**2 * gauss)
        drag_coeff = 0.0 if np.isclose(alpha, 0.0) else OPTIMAL_LAMBDA_DRAG * At_dot / alpha
        
        At_vec[i] = At
        
        H_mid = Operator(
            basis_to_matrix={"energy" : transmon.H0["energy"] + At * transmon.n["energy"] + (drag_coeff * n_12_y)}
        )
        ...
        if LIVE_VISUALIZATION and i % update_interval == 0:
            ...
            line_drive.set_data(t_ns_so_far, At_vec[:i+1] / (h * 1e9))
    ...
    if LIVE_VISUALIZATION:
        ...
        t_ns = t_vec * 1e9
        fig, (ax1, ax2) = plt.subplots(2, 1, sharex=True, figsize=(12, 8))
        ax1.plot(t_ns, At_vec / (h * 1e9), color="tab:blue", linewidth=2)
        ax1.set_ylabel("Drive Amplitude (GHz)", fontsize=14)
        ax1.set_title("Crank-Nicolson SFQ Drive Simulation", fontsize=16)
        ax1.tick_params(labelsize=12)
